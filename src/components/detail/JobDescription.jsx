    ...
    <section className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2">
      <div className="glass-panel p-6">
        <h2 className="mb-4 flex items-center gap-2 text-lg font-bold text-slate-100">
          <ListChecks size={19} className="text-purple-400" />
      ...
      <div className="glass-panel p-6">
        ...
        <ul className="space-y-3">
          {examples.map((example, idx) => (
            <li
              key={idx}
              className="rounded-lg border border-purple-500/10 bg-purple-500/5 px-4 py-3 text-sm leading-relaxed text-slate-400"
            >
              {example}
            </li>
          ))}
